<x-app-layout>
    <div class="mt-[50px] px-4">
        ...
        <div class="mt-8 mb-4">
            <form method="GET" action="{{ route('admin.users') }}" id="filterForm" class="flex justify-between items-center flex-wrap gap-4">
                <div class="flex gap-2 max-w-md flex-1">
                    ...
                    @if(request('search'))
                    <a href="{{ route('admin.users', ['with_trashed' => request('with_trashed')]) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded">Сбросить</a>
                    @endif
                </div>
                <label class="inline-flex items-center">
                    <input type="checkbox" name="with_trashed" value="1" {{ request('with_trashed') ? 'checked' : '' }}
                        onchange="this.form.submit();" class="mr-2">
                    <span class="text-gray-700 dark:text-gray-300">Показать удалённых</span>
                </label>
            </form>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white dark:bg-gray-800">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-200 dark:border-gray-700 {{ $user->trashed() ? 'bg-gray-100 dark:bg-gray-900 opacity-75' : '' }}">
